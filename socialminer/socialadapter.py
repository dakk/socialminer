class Resource:
	...
	def serialize (self):
		return { 'datetime': str (self.datetime), 'type': self.type, 'id': self.ID, 'link': self.link, 'body': self.body, 'confidence': self.confidence }

	def __repr__ (self):
		return "<Resource DateTime=%s Type=%s ID=%s Link=%s Body=%s Confidence=%f>" % (str(self.datetime), self.type, self.ID, self.link, self.body, self.confidence)
...
class Report:
	...
	def __repr__ (self):
		r = "<%sReport User=%s DateTime=%s Avatar=%s Confidence=%s Geolocalization=%s Resources=%d>\n" % (self.adapter, self.user, str(self.datetime), self.avatar, self.confidence, self.geolocalization, len(self.resources))
		for res in self.resources:
